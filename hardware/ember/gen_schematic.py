#!/usr/bin/env python3
"""Generate ember.kicad_sch from the routed board.

Nets are read out of ember.kicad_pcb rather than typed again, so the schematic
...
Usage:  <kicad-python> gen_schematic.py ember.kicad_pcb ember.kicad_sch <Espressif.kicad_sym>
"""
...
for ref, val, lib_id, nets in parts:
    ...
    (property "Value" "{val}" (at {sx+6.5} {sy-4.0} 0)
      (effects (font (size 1.27 1.27)) (justify left)))
    (instances (project "ember" (path "/{uid("sheet")}" (reference "{ref}") (unit 1))))
  )''')
...
with open(OUT, "w") as f:
    f.write(f'''(kicad_sch (version 20231120) (generator "ember_gen") (uuid "{uid("sch")}")
  (paper "A2")
  (title_block
